...
def Complete( base ):
  ...
  for i, x in enumerate( completions ):
    if i % 1500 == 0: # TODO: improve performance
...
def TriggerSnip(): ###
  ...
  query = Vim.Line[ State.complete_start : backup_colnum ]
  pre, post = Vim.Line[ : State.complete_start ], Vim.Line[ backup_colnum : ]


  # vim has prepared to get placeholder for query.
  completions = State.completions

  # search only one complete_data.
  # TODO: c++ function overrides have not support yet.
  ...
  if not completion_info or not completion_info.has_placeholder():
    ...
    if _FeedKeysForCompletions( completions, query ):
      return 1
    else:
...
def _FeedKeysForCompletions( completions, query ):
  ...
  and completions[ left ].spelling == completions[ right-1 ].spelling:
    ...
    Vim.feedkeys( completions[ left ].spelling, 'n' )
    return True
