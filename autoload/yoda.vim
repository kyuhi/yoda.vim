...
let g:yoda_snippet_trigger_key =
\  get( g:, 'yoda_snippet_trigger_key', "<Tab>" )
...
func! yoda#on_filetype()
  ...
  " set mapping
  let b:yoda_snip_map = s:map_store_new( g:yoda_snippet_trigger_key, 'i' )
...
func! yoda#omni_complete( findstart, base )
  if a:findstart
    ...
    if do_map
      ...
      call b:yoda_snip_map.push( s:printf( '<c-r>=<SID>%s()<return>', funcname ) )
      " let options = '<silent> <buffer>'
      " exec s:printf( "silent! inoremap %s %s <C-r>=<SID>%s()<CR>",
      " \              options,
      " \              g:yoda_snippet_trigger_key,
      " \              funcname )
    endif
    YodaPY vim.command(
...
func! s:init_python_module()
  ...
  if exists( 's:loaded_python' ) " is this neccesarry?
    call s:echohl( 'ModeMsg', 'python already loaded' )
    return 1
  endif

  " initialize python module for yoda.
  " return 0 if failure
  ...
  let s:loaded_python = 1 " is this neccesarry?
